...
class Piece:
    def dummy_function(self, *args): # A function that returns an empty list. Use to steralise a Piece object or in definition of an empty Piece
        return [[]]

    def sterilise(self): # destroy the piece. All ouputs are of appropriate format, but are useless. Move function is set to be the dummy function
        self.has_moved = None
        self.colour = '-'
        self.type = '-'
        self.code = '--'
    ...
    def __init__(self, piece_code, value, pos, move_function_pointer):
        ...
        # if a pointer is provided, use it, else set the function to be the dummy function
...
class Lib:
    ...
    def print_board(self, board): # print the board in terms of piece codes in a formatted grid
        print([[piece.code for piece in row] for row in board])

    def next_player(self): # shift to the next player
    ...
    def get_moves(self, board, piece_dict, colour, threatened_only): # return available moves in a singular list of tuple pairs not segregated by separate pieces [(start_pos), (end_pos), ...]
        ...
        for code in (self.PIECE_CODES[:6] if colour == 'w' else self.PIECE_CODES[6:]): # loop over piece codes for relevant colour
            for piece in piece_dict[code]: # access every piece that exists for each code
                piece_moves = piece.get_moves(board, piece, threatened_only, self.moves_made[-1]) # retrieve piece moves
                moves += piece_moves

        moves = [move for move in moves if move != []] # discard all empty lists
    ...
    def move_is_valid(self, board, move):
        ...
        for piece_move in piece_moves:
            if piece_move == move: # test all indicies. Player move has been formatted to provide full special moves
                return True
    ...
    def move_piece(self, board, move):
        ...
        if size == 3: # if move is size 3, en pasan is at play. Destroy piece referenced in third index
            ...
            print('castling')
            rook_start_pos, rook_end_pos = move[2:4]
